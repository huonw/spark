...
class DataFrameSuite extends QueryTest with SharedSQLContext {
  ...
  test("Star Expansion - hash64") {
    val structDf = testData2.select("a", "b").as("record")
    checkAnswer(
      structDf.groupBy($"a", $"b").agg(min(hash64($"a", $"*"))),
      structDf.groupBy($"a", $"b").agg(min(hash64($"a", $"a", $"b"))))

    checkAnswer(
      structDf.groupBy($"a", $"b").agg(hash64($"a", $"*")),
      structDf.groupBy($"a", $"b").agg(hash64($"a", $"a", $"b")))

    checkAnswer(
      structDf.select(hash64($"*")),
      structDf.select(hash64($"record.*")))

    checkAnswer(
      structDf.select(hash64($"a", $"*")),
      structDf.select(hash64($"a", $"record.*")))
  }
